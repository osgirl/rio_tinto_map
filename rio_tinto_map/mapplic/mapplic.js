...
(function($) {
	...
	var Mapplic = function() {
		...
		function InitWarning() {
			...
			this.init = function() {
				this.bg = $('<div></div>').addClass('init-warning');
				self.map.append(this.bg);
				
				$('.mapplic-layer').css('opacity', 0.5);
				
				this.el = $('<div></div>').addClass('warning-window');
				
				$('<a></a>').addClass('warning-window-close').attr('href', '#').click(function(e) {
					
					$('.mapplic-layer').css('opacity', 1);
					$('.warning-window').stop().fadeOut(300);
					$('.init-warning').stop().fadeOut(300);

				}).appendTo(this.el);
				
				$('<h1>Rio Tinto Kennecott Foundational Behaviors and Operating systems module</h1>').appendTo(this.el);
				$('<p>Welcome to Kennecott’s Foundational Behaviors and operating systems module.</br>Here, you will learn how Kennecott overcame the Manefay slide and made cultural and operational changes across the business, turning an unprecedented challenge into opportunity. Kennecott has made significant progress on behavioral change leading to improved safety performance, surpassed production targets, and significantly reduced costs through a leaner and more efficient way of working.</p>').appendTo(this.el);
				$('<p>Employees are demonstrating a sustainable change through embedded foundational behaviors  and operating systems. Hear from Kennecott’s leaders who describe how employees are being engaged and delivering safety, cultural and productivity gains through:</p>').appendTo(this.el);
				$('<ul><li>Integrated operations and collaboration</li><li>Leadership in the field</li><li>Standard Work</li><li>Kaizens</li><li>Mandates</li></ul>').appendTo(this.el);
				$('<br><p>Click on the operating systems wheel, videos and additional material to learn more.</p>').appendTo(this.el);
		...
		function GoalWindow() {
			...
			this.init = function() {
				this.el = $('<div></div>').addClass('goal-window');
				
				$('<a></a>').addClass('goal-window-close').attr('href', '#').click(function(e) {
					
					$('.mapplic-layer').css('opacity', 1);
					$('.goal-window').stop().fadeOut(300);
					$('.init-warning').stop().fadeOut(300);
			...
			this.show = function() {
				$('.mapplic-layer').css('opacity', 0.5);
				$('.goal-window').stop().fadeIn(300);
				$('.init-warning').stop().fadeIn(300);
			}
		}
		...
		function Tooltip() {
			...
			this.set = function(location) {
				if (location) {
					...
					$('.mapplic-hovertip').css('display', 'none');
			...
			this.position = function(location) {
				...
				this.el.css({
					...
					top: y + '%',
					marginTop: mt,
					marginLeft: ml + 10 - location.xshift
				});
		...
		function HoverTooltip() {
			...
			this.init = function() {
				...
				// Construct
				this.el = $('<div></div>').addClass('mapplic-hovertip');
				this.photo = $('<img>').appendTo(this.el);
				//this.title = $('<h4></h4>').addClass('mapplic-tooltip-title').appendTo(this.el);
				//$('<div></div>').addClass('mapplic-tooltip-triangle').appendTo(this.el);

				// Events
				$(self.map).on('mouseover', '.mapplic-layer .hover', function() {
					var data = '';
					...
					if(location.hover == 'yes') {
						if($('.mapplic-tooltip').css('display') == 'block') {
						} else {
			...
			this.show = function(location) {
				//this.title.text(location.title);
				this.photo.attr('src', location.photo);
				var x = location.x * 100,
					y = location.y * 100,
					mt = -this.el.outerHeight() - this.shift,
					ml = -this.el.outerWidth() / 2;
		...
		function ClearButton() {
			...
			this.init = function() {
				this.el = $('<div></div>').addClass('mapplic-clear');
				$('<a></a>').attr('href', '#').addClass('mapplic-clear-button').click(function(e) {
					e.preventDefault();
					self.deeplinking.clear();
					self.tooltip.hide();
					zoomTo(0.5, 0.5, 1);
				}).appendTo(this.el);
				this.el.appendTo(self.container);
			}
		}
		...
		var processData = function(data) {
				...
				$.each(data.levels, function(index, value) {
					...
					switch (extension) {
						...
						case 'jpg': case 'jpeg': case 'png': case 'gif':
							$('<img>').attr({src: source, usemap: '#maphighlight'}).addClass('mapplic-map-image').appendTo(layer);
							
							var map_image = $('<map>').attr('name', 'maphighlight');
							
							$.each(value.locations, function(index, value) {
							
								$('<area>').addClass('hover').attr({
									shape: 'rect',
									coords: value.area,
									href: '#' + value.id
								}).appendTo(map_image);
			...
			self.initwarning = new InitWarning();
			self.initwarning.init();
			
			self.goalwindow = new GoalWindow();
			self.goalwindow.init();
